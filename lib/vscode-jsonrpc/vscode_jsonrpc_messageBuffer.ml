...
module AbstractMessageBuffer =
  struct
    ...
    let (create : ?encoding:RAL.MessageBufferEncoding.t -> unit -> t) =
      fun ?encoding:(x12 : RAL.MessageBufferEncoding.t option) ->
        fun () ->
          t_of_js
            (Ojs.new_obj_arr
               ...
               (let x13 =
                ...
                (match x12 with
                 | Some x14 ->
                     ignore
                       (Ojs.call x13 "push"
                          [|(RAL.MessageBufferEncoding.t_to_js x14)|])
                 | None -> ());
                x13))
    ...
    let (fromString :
      t ->
        value:string -> encoding:RAL.MessageBufferEncoding.t -> Uint8Array.t)
      =
      fun (x18 : t) ->
        fun ~value:(x16 : string) ->
          fun ~encoding:(x17 : RAL.MessageBufferEncoding.t) ->
            Uint8Array.t_of_js
              (Ojs.call (t_to_js x18) "fromString"
                 [|(Ojs.string_to_js x16);(RAL.MessageBufferEncoding.t_to_js
                                             x17)|])
    let (toString :
      t ->
        value:Uint8Array.t -> encoding:RAL.MessageBufferEncoding.t -> string)
      =
      fun (x21 : t) ->
        fun ~value:(x19 : Uint8Array.t) ->
          fun ~encoding:(x20 : RAL.MessageBufferEncoding.t) ->
            Ojs.string_of_js
              (Ojs.call (t_to_js x21) "toString"
                 [|(Uint8Array.t_to_js x19);(RAL.MessageBufferEncoding.t_to_js
                                               x20)|])
    let (asNative :
    ...
    let (get_encoding : t -> RAL.MessageBufferEncoding.t) =
      fun (x30 : t) ->
    ...
    let (tryReadHeaders : t -> (string, string) Map.t_2 or_undefined) =
      fun (x34 : t) ->
        or_undefined_of_js
          (fun (x35 : Ojs.t) ->
             Map.t_2_of_js Ojs.string_of_js Ojs.string_of_js x35)
    ...
    let (cast : t -> RAL.MessageBuffer.t) =
      fun (x45 : t) -> RAL.MessageBuffer.t_of_js (t_to_js x45)
  end
