...
module Touch =
  struct
    type t = _Touch
    let rec t_of_js : Ojs.t -> t = fun (x10 : Ojs.t) -> _Touch_of_js x10
    and t_to_js : t -> Ojs.t = fun (x9 : _Touch) -> _Touch_to_js x9
  end
module LinkedMap =
  struct
                 ...
                 Ojs.call (Ojs.get_prop_ascii x123 "forEach") "apply"
                   [|x123;((let x113 =
                            ...
                            (match x112 with
                             ...
                             | None -> ());
                            x113))|])
    let (keys : ('K, 'V) t -> 'K IterableIterator.t_1) =
      fun (x124 : ('K, 'V) t) ->
        IterableIterator.t_1_of_js Obj.magic
          (Ojs.call (t_to_js Obj.magic Obj.magic x124) "keys" [||])
    let (values : ('K, 'V) t -> 'V IterableIterator.t_1) =
      fun (x128 : ('K, 'V) t) ->
        IterableIterator.t_1_of_js Obj.magic
          (Ojs.call (t_to_js Obj.magic Obj.magic x128) "values" [||])
    let (entries : ('K, 'V) t -> ('K * 'V) IterableIterator.t_1) =
      fun (x132 : ('K, 'V) t) ->
    ...
    let (_Symbol_iterator_ : ('K, 'V) t -> ('K * 'V) IterableIterator.t_1) =
      fun (x137 : ('K, 'V) t) ->
        IterableIterator.t_1_of_js
          (fun (x140 : Ojs.t) ->
             let x141 = x140 in
            ...
            (Ojs.call (t_to_js Obj.magic Obj.magic x184) "fromJSON"
               [|(Ojs.list_to_js
                    (fun (x180 : ('K * 'V)) ->
                       ...
                       x183) x179)|])
    let (cast : ('K, 'V) t -> ('K, 'V) Map.t_2) =
      fun (x187 : ('K, 'V) t) ->
        Map.t_2_of_js Obj.magic Obj.magic (t_to_js Obj.magic Obj.magic x187)
  end
module LRUCache =
  struct
