...
module DataCallback =
  struct
    ...
    let (apply : t -> data:Message.t -> unit) =
      fun (x4 : t) ->
        fun ~data:(x3 : Message.t) ->
          ignore (Ojs.apply (t_to_js x4) [|(Message.t_to_js x3)|])
  end
...
module MessageReader =
  struct
    ...
    let (get_onError : t -> Error.t Event.t_1) =
      fun (x11 : t) ->
        Event.t_1_of_js Error.t_of_js
    ...
    let (get_onPartialMessage : t -> _PartialMessageInfo Event.t_1) =
      fun (x15 : t) ->
        Event.t_1_of_js _PartialMessageInfo_of_js
          (Ojs.get_prop_ascii (t_to_js x15) "onPartialMessage")
    let (listen : t -> callback:_DataCallback -> Disposable.t) =
      fun (x18 : t) ->
...
module MessageReaderOptions =
  struct
    ...
    let (set_charset : t -> RAL.MessageBufferEncoding.t -> unit) =
      fun (x53 : t) ->
        fun (x54 : RAL.MessageBufferEncoding.t) ->
          Ojs.set_prop_ascii (t_to_js x53) "charset"
            (RAL.MessageBufferEncoding.t_to_js x54)
    let (get_contentDecoder : t -> ContentDecoder.t) =
      fun (x55 : t) ->
        ContentDecoder.t_of_js
          (Ojs.get_prop_ascii (t_to_js x55) "contentDecoder")
    let (set_contentDecoder : t -> ContentDecoder.t -> unit) =
      fun (x56 : t) ->
        fun (x57 : ContentDecoder.t) ->
          Ojs.set_prop_ascii (t_to_js x56) "contentDecoder"
            (ContentDecoder.t_to_js x57)
    let (get_contentDecoders : t -> ContentDecoder.t list) =
      fun (x58 : t) ->
        Ojs.list_of_js ContentDecoder.t_of_js
          (Ojs.get_prop_ascii (t_to_js x58) "contentDecoders")
    let (set_contentDecoders : t -> ContentDecoder.t list -> unit) =
      fun (x60 : t) ->
        fun (x61 : ContentDecoder.t list) ->
          Ojs.set_prop_ascii (t_to_js x60) "contentDecoders"
            (Ojs.list_to_js ContentDecoder.t_to_js x61)
    let (get_contentTypeDecoder : t -> ContentTypeDecoder.t) =
      fun (x63 : t) ->
        ContentTypeDecoder.t_of_js
          (Ojs.get_prop_ascii (t_to_js x63) "contentTypeDecoder")
    let (set_contentTypeDecoder : t -> ContentTypeDecoder.t -> unit) =
      fun (x64 : t) ->
        fun (x65 : ContentTypeDecoder.t) ->
          Ojs.set_prop_ascii (t_to_js x64) "contentTypeDecoder"
            (ContentTypeDecoder.t_to_js x65)
    let (get_contentTypeDecoders : t -> ContentTypeDecoder.t list) =
      fun (x66 : t) ->
        Ojs.list_of_js ContentTypeDecoder.t_of_js
          (Ojs.get_prop_ascii (t_to_js x66) "contentTypeDecoders")
    let (set_contentTypeDecoders : t -> ContentTypeDecoder.t list -> unit) =
      fun (x68 : t) ->
        fun (x69 : ContentTypeDecoder.t list) ->
          Ojs.set_prop_ascii (t_to_js x68) "contentTypeDecoders"
            (Ojs.list_to_js ContentTypeDecoder.t_to_js x69)
  end
    ...
    let (create :
      readable:RAL.ReadableStream.t ->
        ?options:(_MessageReaderOptions, RAL.MessageBufferEncoding.t) union2
          -> unit -> t)
      =
      fun ~readable:(x97 : RAL.ReadableStream.t) ->
        fun
          ?options:(x98 :
                     (_MessageReaderOptions, RAL.MessageBufferEncoding.t)
                       union2 option)
          ->
          fun () ->
            t_of_js
              (Ojs.new_obj_arr
                 ...
                 (let x99 =
                  ...
                  ignore
                    (Ojs.call x99 "push" [|(RAL.ReadableStream.t_to_js x97)|]);
                  (match x98 with
                   | Some x100 ->
                       ignore
                         (Ojs.call x99 "push"
                            [|(union2_to_js _MessageReaderOptions_to_js
                                 RAL.MessageBufferEncoding.t_to_js x100)|])
                   | None -> ());
                  x99))
