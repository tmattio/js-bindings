...
module CancellationToken =
  struct
    ...
    let (get_onCancellationRequested : t -> any Event.t_1) =
      fun (x4 : t) ->
  ...
  struct
    let (none : _CancellationToken) =
      _CancellationToken_of_js
        (Ojs.get_prop_ascii
           (Ojs.get_prop_ascii Ojs.global "CancellationToken") "None")
    let (cancelled : _CancellationToken) =
      _CancellationToken_of_js
        (Ojs.get_prop_ascii
           (Ojs.get_prop_ascii Ojs.global "CancellationToken") "Cancelled")
    let (is : value:any -> bool) =
      fun ~value:(x6 : any) ->
        Ojs.bool_of_js
          (Ojs.call (Ojs.get_prop_ascii Ojs.global "CancellationToken") "is"
             [|(any_to_js x6)|])
  end
module AbstractCancellationTokenSource =
  struct
    ...
    let (cast : t -> Disposable.t) =
      fun (x13 : t) -> Disposable.t_of_js (t_to_js x13)
  end
