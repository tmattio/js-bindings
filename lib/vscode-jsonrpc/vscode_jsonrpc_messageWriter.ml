...
module MessageWriter =
  struct
        ...
        Event.t_1_of_js
          (fun (x4 : Ojs.t) ->
             ...
             ((Error.t_of_js (Ojs.array_get x5 0)),
               (or_undefined_of_js Message.t_of_js (Ojs.array_get x5 1)),
    ...
    let (write : t -> msg:Message.t -> unit Promise.t) =
      fun (x11 : t) ->
        fun ~msg:(x10 : Message.t) ->
          Promise.t_of_js Ojs.unit_of_js
            (Ojs.call (t_to_js x11) "write" [|(Message.t_to_js x10)|])
    let (end_ : t -> unit) =
        ...
        Event.t_1_of_js
          (fun (x26 : Ojs.t) ->
             ...
             ((Error.t_of_js (Ojs.array_get x27 0)),
               (or_undefined_of_js Message.t_of_js (Ojs.array_get x27 1)),
               (or_undefined_of_js Ojs.float_of_js (Ojs.array_get x27 2))))
          (Ojs.get_prop_ascii (t_to_js x25) "onError")
    let (fireError :
      t -> error:any -> ?message:Message.t -> ?count:float -> unit -> unit) =
      fun (x36 : t) ->
        fun ~error:(x30 : any) ->
          fun ?message:(x31 : Message.t option) ->
...
module MessageWriterOptions =
  struct
    ...
    let (set_charset : t -> RAL.MessageBufferEncoding.t -> unit) =
      fun (x47 : t) ->
        fun (x48 : RAL.MessageBufferEncoding.t) ->
          Ojs.set_prop_ascii (t_to_js x47) "charset"
            (RAL.MessageBufferEncoding.t_to_js x48)
    let (get_contentEncoder : t -> ContentEncoder.t) =
      fun (x49 : t) ->
        ContentEncoder.t_of_js
          (Ojs.get_prop_ascii (t_to_js x49) "contentEncoder")
    let (set_contentEncoder : t -> ContentEncoder.t -> unit) =
      fun (x50 : t) ->
        fun (x51 : ContentEncoder.t) ->
          Ojs.set_prop_ascii (t_to_js x50) "contentEncoder"
            (ContentEncoder.t_to_js x51)
    let (get_contentTypeEncoder : t -> ContentTypeEncoder.t) =
      fun (x52 : t) ->
        ContentTypeEncoder.t_of_js
          (Ojs.get_prop_ascii (t_to_js x52) "contentTypeEncoder")
    let (set_contentTypeEncoder : t -> ContentTypeEncoder.t -> unit) =
      fun (x53 : t) ->
        fun (x54 : ContentTypeEncoder.t) ->
          Ojs.set_prop_ascii (t_to_js x53) "contentTypeEncoder"
            (ContentTypeEncoder.t_to_js x54)
  end
    ...
    let (create :
      writable:RAL.WritableStream.t ->
        ?options:(_MessageWriterOptions, RAL.MessageBufferEncoding.t) union2
          -> unit -> t)
      =
      fun ~writable:(x69 : RAL.WritableStream.t) ->
        fun
          ?options:(x70 :
                     (_MessageWriterOptions, RAL.MessageBufferEncoding.t)
                       union2 option)
          ->
          fun () ->
            t_of_js
              (Ojs.new_obj_arr
                 ...
                 (let x71 =
                  ...
                  ignore
                    (Ojs.call x71 "push" [|(RAL.WritableStream.t_to_js x69)|]);
                  (match x70 with
                   | Some x72 ->
                       ignore
                         (Ojs.call x71 "push"
                            [|(union2_to_js _MessageWriterOptions_to_js
                                 RAL.MessageBufferEncoding.t_to_js x72)|])
                   | None -> ());
                  x71))
    let (write : t -> msg:Message.t -> unit Promise.t) =
      fun (x76 : t) ->
        fun ~msg:(x75 : Message.t) ->
          Promise.t_of_js Ojs.unit_of_js
            (Ojs.call (t_to_js x76) "write" [|(Message.t_to_js x75)|])
