[@@@js.dummy "!! This code has been generated by gen_js_api !!"]
[@@@ocaml.warning "-7-32-39"]
[@@@ocaml.warning "-7-11-32-33-39"]
open Es5
module Internal =
  struct
    module AnonymousInterfaces = struct  end
    module Types =
      struct
        open AnonymousInterfaces
        type _ImplementationFeature = [ `ImplementationFeature ] intf
        and _ImplementationMiddleware = [ `ImplementationMiddleware ] intf
        and _ProvideImplementationSignature =
          [ `ProvideImplementationSignature ] intf
        let rec _ImplementationFeature_of_js :
          Ojs.t -> _ImplementationFeature = Obj.magic
        and _ImplementationFeature_to_js : _ImplementationFeature -> Ojs.t =
          Obj.magic
        and _ImplementationMiddleware_of_js :
          Ojs.t -> _ImplementationMiddleware = Obj.magic
        and _ImplementationMiddleware_to_js :
          _ImplementationMiddleware -> Ojs.t = Obj.magic
        and _ProvideImplementationSignature_of_js :
          Ojs.t -> _ProvideImplementationSignature = Obj.magic
        and _ProvideImplementationSignature_to_js :
          _ProvideImplementationSignature -> Ojs.t = Obj.magic
      end
  end
open Internal
open AnonymousInterfaces
open Types
module ProvideImplementationSignature =
  struct
    type t = _ProvideImplementationSignature
    let rec t_of_js : Ojs.t -> t =
      fun (x2 : Ojs.t) -> _ProvideImplementationSignature_of_js x2
    and t_to_js : t -> Ojs.t =
      fun (x1 : _ProvideImplementationSignature) ->
        _ProvideImplementationSignature_to_js x1
    let (apply :
      t ->
        this:unit ->
          document:TextDocument.t ->
            position:VPosition.t ->
              token:CancellationToken.t ->
                (VDefinition.t, VDefinitionLink.t) or_array
                  ProviderResult.t_1)
      =
      fun (x7 : t) ->
        fun ~this:(x3 : unit) ->
          fun ~document:(x4 : TextDocument.t) ->
            fun ~position:(x5 : VPosition.t) ->
              fun ~token:(x6 : CancellationToken.t) ->
                ProviderResult.t_1_of_js
                  (fun (x8 : Ojs.t) ->
                     or_array_of_js VDefinition.t_of_js
                       VDefinitionLink.t_of_js x8)
                  (Ojs.apply (t_to_js x7)
                     [|(Ojs.unit_to_js x3);(TextDocument.t_to_js x4);(
                       VPosition.t_to_js x5);(CancellationToken.t_to_js x6)|])
  end
module ImplementationMiddleware =
  struct
    type t = _ImplementationMiddleware
    let rec t_of_js : Ojs.t -> t =
      fun (x12 : Ojs.t) -> _ImplementationMiddleware_of_js x12
    and t_to_js : t -> Ojs.t =
      fun (x11 : _ImplementationMiddleware) ->
        _ImplementationMiddleware_to_js x11
    let (provideImplementation :
      t ->
        this:unit ->
          document:TextDocument.t ->
            position:VPosition.t ->
              token:CancellationToken.t ->
                next:_ProvideImplementationSignature ->
                  (VDefinition.t, VDefinitionLink.t) or_array
                    ProviderResult.t_1)
      =
      fun (x18 : t) ->
        fun ~this:(x13 : unit) ->
          fun ~document:(x14 : TextDocument.t) ->
            fun ~position:(x15 : VPosition.t) ->
              fun ~token:(x16 : CancellationToken.t) ->
                fun ~next:(x17 : _ProvideImplementationSignature) ->
                  ProviderResult.t_1_of_js
                    (fun (x19 : Ojs.t) ->
                       or_array_of_js VDefinition.t_of_js
                         VDefinitionLink.t_of_js x19)
                    (Ojs.call (t_to_js x18) "provideImplementation"
                       [|(Ojs.unit_to_js x13);(TextDocument.t_to_js x14);(
                         VPosition.t_to_js x15);(CancellationToken.t_to_js
                                                   x16);(_ProvideImplementationSignature_to_js
                                                           x17)|])
  end
module ImplementationFeature =
  struct
    type t = _ImplementationFeature
    let rec t_of_js : Ojs.t -> t =
      fun (x23 : Ojs.t) -> _ImplementationFeature_of_js x23
    and t_to_js : t -> Ojs.t =
      fun (x22 : _ImplementationFeature) -> _ImplementationFeature_to_js x22
    let (create : client:BaseLanguageClient.t -> t) =
      fun ~client:(x24 : BaseLanguageClient.t) ->
        t_of_js
          (Ojs.new_obj
             (Ojs.get_prop_ascii Ojs.global "ImplementationFeature")
             [|(BaseLanguageClient.t_to_js x24)|])
    let (fillClientCapabilities :
      t -> capabilities:ClientCapabilities.t -> unit) =
      fun (x26 : t) ->
        fun ~capabilities:(x25 : ClientCapabilities.t) ->
          ignore
            (Ojs.call (t_to_js x26) "fillClientCapabilities"
               [|(ClientCapabilities.t_to_js x25)|])
    let (initialize :
      t ->
        capabilities:ServerCapabilities.t ->
          documentSelector:DocumentSelector.t -> unit)
      =
      fun (x29 : t) ->
        fun ~capabilities:(x27 : ServerCapabilities.t) ->
          fun ~documentSelector:(x28 : DocumentSelector.t) ->
            ignore
              (Ojs.call (t_to_js x29) "initialize"
                 [|(ServerCapabilities.t_to_js x27);(DocumentSelector.t_to_js
                                                       x28)|])
    let (registerLanguageProvider :
      t ->
        options:ImplementationRegistrationOptions.t ->
          (Disposable.t * ImplementationProvider.t))
      =
      fun (x31 : t) ->
        fun ~options:(x30 : ImplementationRegistrationOptions.t) ->
          let x32 =
            Ojs.call (t_to_js x31) "registerLanguageProvider"
              [|(ImplementationRegistrationOptions.t_to_js x30)|] in
          ((Disposable.t_of_js (Ojs.array_get x32 0)),
            (ImplementationProvider.t_of_js (Ojs.array_get x32 1)))
    let (cast :
      t ->
        (ImplementationOptions.t or_boolean,
          ImplementationRegistrationOptions.t, ImplementationProvider.t)
          TextDocumentFeature.t_3)
      =
      fun (x33 : t) ->
        TextDocumentFeature.t_3_of_js
          (fun (x34 : Ojs.t) ->
             or_boolean_of_js ImplementationOptions.t_of_js x34)
          ImplementationRegistrationOptions.t_of_js
          ImplementationProvider.t_of_js (t_to_js x33)
  end
