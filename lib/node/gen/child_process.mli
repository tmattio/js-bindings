...
module Child_process : sig
  ...
  module ChildProcess : sig
    ...
    val get_stdin : t -> Stream.Stream.Writable.t or_null [@@js.get "stdin"]

    val set_stdin : t -> Stream.Stream.Writable.t or_null -> unit
      [@@js.set "stdin"]

    val get_stdout : t -> Stream.Stream.Readable.t or_null [@@js.get "stdout"]

    val set_stdout : t -> Stream.Stream.Readable.t or_null -> unit
      [@@js.set "stdout"]

    val get_stderr : t -> Stream.Stream.Readable.t or_null [@@js.get "stderr"]

    val set_stderr : t -> Stream.Stream.Readable.t or_null -> unit
      [@@js.set "stderr"]

    val get_channel : t -> Stream.Stream.Pipe.t or_null [@@js.get "channel"]

    val get_stdio
      :  t
      -> Stream.Stream.Writable.t or_null
         * Stream.Stream.Readable.t or_null
         * Stream.Stream.Readable.t or_null
    ...
    val get_signalCode : t -> Process.Process.Signals.t or_null
      [@@js.get "signalCode"]
    ...
    val kill : t -> ?signal:Process.Process.Signals.t or_number -> unit -> bool
      [@@js.call "kill"]
  ...
  module ChildProcessWithoutNullStreams : sig
    ...
    val get_stdin : t -> Stream.Stream.Writable.t [@@js.get "stdin"]

    val set_stdin : t -> Stream.Stream.Writable.t -> unit [@@js.set "stdin"]

    val get_stdout : t -> Stream.Stream.Readable.t [@@js.get "stdout"]

    val set_stdout : t -> Stream.Stream.Readable.t -> unit [@@js.set "stdout"]

    val get_stderr : t -> Stream.Stream.Readable.t [@@js.get "stderr"]

    val set_stderr : t -> Stream.Stream.Readable.t -> unit [@@js.set "stderr"]

    val get_stdio
      :  t
      -> Stream.Stream.Writable.t
  ...
  module ProcessEnvOptions : sig
    ...
    val get_env : t -> Process.Process.ProcessEnv.t [@@js.get "env"]

    val set_env : t -> Process.Process.ProcessEnv.t -> unit [@@js.set "env"]
  end
  ...
  module ExecOptions : sig
    ...
    val get_killSignal : t -> Process.Process.Signals.t or_number
      [@@js.get "killSignal"]

    val set_killSignal : t -> Process.Process.Signals.t or_number -> unit
      [@@js.set "killSignal"]
  ...
  module ExecException : sig
    ...
    val get_signal : t -> Process.Process.Signals.t [@@js.get "signal"]

    val set_signal : t -> Process.Process.Signals.t -> unit [@@js.set "signal"]
  ...
  val exec
    :  command:string
    -> options:
         (Fs.Fs.BaseEncodingOptions.t, child_process_ExecOptions) intersection2
         or_null_or_undefined
    -> ?callback:
         (error:child_process_ExecException or_null
          -> stdout:Buffer.t or_string
  ...
  module SpawnSyncReturns : sig
    ...
    val get_signal : 'T t -> Process.Process.Signals.t or_null
      [@@js.get "signal"]

    val set_signal : 'T t -> Process.Process.Signals.t or_null -> unit
      [@@js.set "signal"]
