...
module AnonymousInterface1 =
  struct
    ...
    let (create : t -> size:float -> Buffer.t_0) =
      fun (x15 : t) ->
        fun ~size:(x14 : float) ->
          Buffer.t_0_of_js
            (Ojs.new_obj (t_to_js x15) [|(Ojs.float_to_js x14)|])
    let (get_prototype : t -> Buffer.t_0) =
      fun (x16 : t) ->
        Buffer.t_0_of_js (Ojs.get_prop_ascii (t_to_js x16) "prototype")
    let (set_prototype : t -> Buffer.t_0 -> unit) =
      fun (x17 : t) ->
        fun (x18 : Buffer.t_0) ->
          Ojs.set_prop_ascii (t_to_js x17) "prototype" (Buffer.t_0_to_js x18)
  end
module Node_buffer = struct  end
module Buffer =
  struct
