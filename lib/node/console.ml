                  ...
                  (let x31 = t_to_js x30 in
                   Ojs.call (Ojs.get_prop_ascii x31 "debug") "apply"
                     [|x31;((let x27 =
                             ...
                             x27))|])
        let (dir :
          t -> obj:any -> ?options:Util.Util.InspectOptions.t -> unit -> unit)
          =
          fun (x36 : t) ->
            fun ~obj:(x32 : any) ->
              fun ?options:(x33 : Util.Util.InspectOptions.t option) ->
                fun () ->
                  ignore
                       ...
                       [|x37;((let x34 =
                               ...
                               (match x33 with
                                | Some x35 ->
                                    ignore
                                      (Ojs.call x34 "push"
                                         [|(Util.Util.InspectOptions.t_to_js
                                              x35)|])
                                | None -> ());
                               x34))|])
                ...
                fun (x143 : [ `auto ] or_boolean) ->
                  Ojs.set_prop_ascii (t_to_js x142) "colorMode"
                    (or_boolean_to_js
                       ...
                       x143)
            let (get_inspectOptions : t -> Util.Util.InspectOptions.t) =
              fun (x145 : t) ->
                Util.Util.InspectOptions.t_of_js
                  (Ojs.get_prop_ascii (t_to_js x145) "inspectOptions")
            let (set_inspectOptions :
              t -> Util.Util.InspectOptions.t -> unit) =
              fun (x146 : t) ->
                fun (x147 : Util.Util.InspectOptions.t) ->
                  Ojs.set_prop_ascii (t_to_js x146) "inspectOptions"
                    (Util.Util.InspectOptions.t_to_js x147)
          end
