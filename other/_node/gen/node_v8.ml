...
module V8 =
  struct
    ...
    let (cachedDataVersionTag : unit -> float) =
      fun () ->
        Ojs.float_of_js
          (Ojs.call (Ojs.get_prop_ascii Ojs.global "v8")
             "cachedDataVersionTag" [||])
    let (getHeapStatistics : unit -> v8_HeapInfo) =
      fun () ->
        v8_HeapInfo_of_js
          (Ojs.call (Ojs.get_prop_ascii Ojs.global "v8") "getHeapStatistics"
             [||])
    ...
    let (setFlagsFromString : flags:string -> unit) =
      fun ~flags:(x70 : string) ->
        ignore
          (Ojs.call (Ojs.get_prop_ascii Ojs.global "v8") "setFlagsFromString"
             [|(Ojs.string_to_js x70)|])
    let (getHeapSnapshot : unit -> Readable.t_0) =
      fun () ->
        Readable.t_0_of_js
          (Ojs.call (Ojs.get_prop_ascii Ojs.global "v8") "getHeapSnapshot"
             [||])
    let (writeHeapSnapshot : ?fileName:string -> unit -> string) =
      fun ?fileName:(x71 : string option) ->
        fun () ->
          Ojs.string_of_js
            (let x74 = Ojs.get_prop_ascii Ojs.global "v8" in
    ...
    module Serializer =
      struct
        ...
        let (releaseBuffer : t -> Buffer.t_0) =
          fun (x80 : t) ->
            Buffer.t_0_of_js (Ojs.call (t_to_js x80) "releaseBuffer" [||])
            ...
            fun ~value:(x89 : float) ->
              ignore
                (Ojs.call (t_to_js x90) "writeDouble"
                   [|(Ojs.float_to_js x89)|])
        let (writeRawBytes : t -> buffer:NodeJS.TypedArray.t -> unit) =
          fun (x92 : t) ->
            fun ~buffer:(x91 : NodeJS.TypedArray.t) ->
              ignore
                (Ojs.call (t_to_js x92) "writeRawBytes"
                   [|(NodeJS.TypedArray.t_to_js x91)|])
      end
    ...
    module Deserializer =
      struct
        ...
        let (create : data:NodeJS.TypedArray.t -> t) =
          fun ~data:(x98 : NodeJS.TypedArray.t) ->
            t_of_js
              (Ojs.new_obj
                 (Ojs.get_prop_ascii (Ojs.get_prop_ascii Ojs.global "v8")
                    "Deserializer") [|(NodeJS.TypedArray.t_to_js x98)|])
        ...
        let (readRawBytes : t -> length:float -> Buffer.t_0) =
          fun (x110 : t) ->
    ...
    let (serialize : value:any -> Buffer.t_0) =
      fun ~value:(x114 : any) ->
        Buffer.t_0_of_js
          (Ojs.call (Ojs.get_prop_ascii Ojs.global "v8") "serialize"
             [|(any_to_js x114)|])
    let (deserialize : data:NodeJS.TypedArray.t -> any) =
      fun ~data:(x115 : NodeJS.TypedArray.t) ->
        any_of_js
          (Ojs.call (Ojs.get_prop_ascii Ojs.global "v8") "deserialize"
             [|(NodeJS.TypedArray.t_to_js x115)|])
  end
