  ...
  struct
    module WASIOptions =
      struct
        ...
        let (get_preopens : t -> string NodeJS.Dict.t_1) =
          fun (x11 : t) ->
            NodeJS.Dict.t_1_of_js Ojs.string_of_js
              (Ojs.get_prop_ascii (t_to_js x11) "preopens")
        let (set_preopens : t -> string NodeJS.Dict.t_1 -> unit) =
          fun (x13 : t) ->
            fun (x14 : string NodeJS.Dict.t_1) ->
              Ojs.set_prop_ascii (t_to_js x13) "preopens"
                (NodeJS.Dict.t_1_to_js Ojs.string_to_js x14)
    ...
    module WASI =
      struct
        ...
        let (create : ?options:wasi_WASIOptions -> unit -> t) =
          fun ?options:(x30 : wasi_WASIOptions option) ->
            fun () ->
              t_of_js
                (Ojs.new_obj_arr
                   (Ojs.get_prop_ascii (Ojs.get_prop_ascii Ojs.global "wasi")
                      "WASI")
                   (let x31 =
        ...
        let (get_wasiImport : t -> any NodeJS.Dict.t_1) =
          fun (x37 : t) ->
            NodeJS.Dict.t_1_of_js any_of_js
